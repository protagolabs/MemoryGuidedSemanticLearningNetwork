...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.co = CoAttention(args.d_model, args.d_model)

        self.memory_v = nn.Parameter(torch.empty(size=(512,args.d_model*2)))
        self.memory_q = nn.Parameter(torch.empty(size=(512,args.d_model*2)))
        nn.init.xavier_uniform(self.memory_v.data, gain=1.414)
        nn.init.xavier_uniform(self.memory_q.data, gain=1.414)

        self.update_v = TanhAttention(args.d_model*2)
        self.update_q = TanhAttention(args.d_model*2)

        self.read_v = NormalSubLayer1(args.d_model*2, args.d_model*2, 4, 0)
        self.read_q = NormalSubLayer1(args.d_model*2, args.d_model*2, 4, 0)

        self.p1 = nn.Linear(args.d_model*2,args.d_model)
        self.p2 = nn.Linear(args.d_model*2,args.d_model)
        self.p3 = nn.Linear(args.d_model,args.d_model)

        self.inner = TanhAttention(args.d_model)
        self.cali = TanhAttention(args.d_model)
        ...
        self.v2s = TanhAttention(args.d_model*3)
        ...
        self.rnn = DynamicGRU(args.d_model *3, args.d_model >> 1, bidirectional=True, batch_first=True)
    ...
    def forward(self, frames, frame_mask, words, word_mask,
        ...
        #attentive
        frames1, x1, _, _ = self.co(frames, x, node_mask)

        #memory
        v_feat = torch.cat([frames1, frames], -1)
        q_feat = torch.cat([x1, x], -1)

        v_feat_ = self.update_v(v_feat,self.memory_v,frame_mask)
        q_feat_ = self.update_q(q_feat,self.memory_q,word_mask)

        self.memory_v, _ = self.read_v(self.memory_v, v_feat_, frame_mask, node_mask)
        self.memory_q, _ = self.read_q(self.memory_q,q_feat_,word_mask,node_mask)

        final_feat1 = self.p1(v_feat_)
        final_feat2, _ = self.inner(self.p1(v_feat_), self.p2(q_feat_), node_mask)
        final_feat3, _ = self.cali(self.p1(v_feat_), self.p3(frames), node_mask)
        ...
        # x1 = self.s2v(x1)
        x = torch.cat([final_feat1,final_feat2, final_feat3], -1) #x1
